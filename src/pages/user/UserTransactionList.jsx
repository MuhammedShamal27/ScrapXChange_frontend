    ...
    <>
      ...
      <div className="flex bg-bgColor">
        ...
        <div className="m-10 bg-white rounded-lg w-full text-sm">
          ...
          {/* Header row with equal spacing */}
          <div className="flex justify-between border-b p-3">
            <h1 className="w-1/4 text-left">Shop Name</h1>
            <h1 className="w-1/4 text-center">Date Picked</h1>
            <h1 className="w-1/4 text-center">Total Quantity</h1>
            <h1 className="w-1/4 text-right">Total Price</h1>
          </div>

          {/* Transaction data row with the same width as the header */}
          {data.map((transaction,index)=>(
          <div key={index} className="flex justify-between p-3 cursor-pointer" onClick={() => handleTransactionClick(transaction)} >
            <h1 className="w-1/4 text-left">{transaction.shop_name}</h1>
            <h1 className="w-1/4 text-center">{new Date(transaction.date_picked).toLocaleDateString()}</h1>
            <h1 className="w-1/4 text-center">{transaction.total_quantity} kg</h1>
            <h1 className="w-1/4 text-right">₹{transaction.total_price}</h1>
          </div>
          ))}
        </div>
      </div>
