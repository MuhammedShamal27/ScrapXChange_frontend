...
import { DetailOfReport, ReportBlockUnblock, ReportReasons } from '../../services/api/admin/adminApi'
                ...
                <div className='flex m-10 gap-10'>
                    <div className='bg-white w-6/12 rounded-lg flex flex-col justify-around'>
                        ...
                        <div className='flex justify-around text-sm p-3'>
                            <button className='rounded-2xl bg-black text-white w-1/6 h-7 '>Go Back</button>
                            {!isBlocked && (
                                <button className='rounded-2xl bg-orange-400 text-white w-2/6 h-7' onClick={handleWarning}>
                                    Make a Warning ({warningCount})
                                </button>
                            )}
                            <button 
                            className={`rounded-2xl ${isBlocked ? 'bg-green-400' : 'bg-red-500'} text-white w-1/6 h-7`} 
                            onClick={isBlocked ? handleUnblock : handleBlock}>{isBlocked ? 'Unblock' : 'Block'}
                            </button>
                    ...
                    <div className='bg-white w-6/12 rounded-lg '>
                        ...
                        <div className='flex flex-col ml-5 p-3'>
                            ...
                            <div className='flex text-sm justify-between'>
                                <p>{reportReason ? reportReason.reason_counts.fraud : "Loading..."}</p>
                                <p>{reportReason ? reportReason.reason_counts.inappropriate : "Loading..."}</p>
                                <p>{reportReason ? reportReason.reason_counts.spam : "Loading..."}</p>
                                <p>{reportReason ? reportReason.reason_counts.other : "Loading..."}</p>
                                <p>{reportReason ? reportReason.total_reports : "Loading..."}</p>
                            </div>
                        </div>
                    </div>
