  ...
  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div
          className="bg-white w-full max-w-4xl rounded-lg p-6 md:p-8 text-sm shadow-lg relative"
        >
          {/* Close Button */}
          <button
            onClick={() => onClose()}
          ...
          <div className="text-center mb-6">
            <h1 className="font-bold text-3xl text-gray-800">Transaction Details</h1>
          </div>
            ...
            <div className="bg-bgColor w-full md:w-1/2 p-4 rounded-lg shadow-sm">
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <p className="text-gray-800">{transaction.collection_request.name}</p>
                </div>
                <div className="flex justify-between">
                  <h3 className="font-medium text-gray-600">Address:</h3>
                  <p className="text-gray-800">{transaction.collection_request.address}</p>
                </div>
                <div className="flex justify-between">
                  <h3 className="font-medium text-gray-600">Pincode:</h3>
                  <p className="text-gray-800">{transaction.collection_request.pincode}</p>
                </div>
                <div className="flex justify-between">
                  <h3 className="font-medium text-gray-600">Phone:</h3>
                  <p className="text-gray-800">{transaction.collection_request.phone}</p>
                </div>
                <div className="flex justify-between">
                  <h3 className="font-medium text-gray-600">Date Requested:</h3>
                  <p className="text-gray-800">{transaction.collection_request.date_requested}</p>
                </div>
                <div className="flex justify-between">
                  <h3 className="font-medium text-gray-600">Date Scheduled:</h3>
                  <p className="text-gray-800">{transaction.collection_request.scheduled_date}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-bgColor w-full md:w-1/2 p-4 rounded-lg shadow-sm">
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <p className="text-gray-800">{transaction.total_quantity} kg</p>
                </div>
                <div className="flex justify-between">
          ...
          <div className="bg-gray-100 w-full mt-6 p-4 rounded-lg shadow-sm">
            <h2 className="font-semibold text-lg text-gray-700 mb-4">Product Information</h2>
            <table className="w-full table-auto">
              <thead>
                <tr>
                  ...
                  <th className="text-left px-4 py-2 text-gray-600">Quantity (kg)</th>
                </tr>
              </thead>
              <tbody>
                {transaction.transaction_products.map((product, index) => (
                  <tr key={index} className="bg-white">
                    <td className="border px-4 py-2 text-gray-800">{product.product.category.name}</td>
                    <td className="border px-4 py-2 text-gray-800">{product.product.name}</td>
                    <td className="border px-4 py-2 text-gray-800">₹{product.product.price}</td>
