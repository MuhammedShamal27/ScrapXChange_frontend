...
import main_profile from "../../assets/SA_profile.png";
...
const UserAndShoplist = ({ list, type, setFilter, setSearchQuery }) => {
  ...
  const handleDetails = (id) => {
    navigate(
      `/admin/${type === "users" ? "userdetails" : "shopdetails"}/${id}`
    );
  };
          ...
          <div className="flex items-center space-x-5">
            ...
            <div className="flex items-center text-sm space-x-2">
              ...
              <Dropdown className="font-semibold" label="Sort" inline>
                ...
                <DropdownItem onClick={() => handleFilterChange("blocked")}>
                  Blocked
                </DropdownItem>
                <DropdownItem onClick={() => handleFilterChange("unblocked")}>
                  UnBlocked
                </DropdownItem>
              </Dropdown>
            </div>
        ...
        <div>
          ...
          <div className="mt-4 space-y-10">
            {currentItems.map((item) => (
              <div
                key={item.id}
                className="grid grid-cols-4 items-center px-10 text-xs"
              >
          ...
          <div className="flex justify-center mt-4 space-x-3">
            {currentPage > 1 && (
              <button
                ...
                className="border rounded-lg bg-white p-2 text-gray-500"
              >
                Prev
              </button>
            )}
